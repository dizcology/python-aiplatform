# Copyright 2021 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

# [START aiplatform_v1beta1_undeploy_model_sample]
from google.cloud import aiplatform_v1beta1


def undeploy_model_sample(
    project: str,
    endpoint_id: str,
    deployed_model_id: str,
    location: str = "us-central1",
    api_endpoint: str = "us-central1-aiplatform.googleapis.com",
    timeout: int = 300,
):
    # The AI Platform services require regional API endpoints.
    client_options = {"api_endpoint": api_endpoint}
    # Initialize client that will be used to create and send requests.
    # This client only needs to be created once, and can be reused for multiple requests.
    client = aiplatform_v1beta1.EndpointServiceClient(client_options=client_options)
    # If after the undeployment there is at least one deployed model remaining in the endpoint, traffic_split should be set to a mapping from remaining deployed models' ids to integer percentages that sum to 100.
    traffic_split = {}
    endpoint = client.endpoint_path(
        project=project, location=location, endpoint=endpoint_id
    )
    response = client.undeploy_model(
        endpoint=endpoint,
        deployed_model_id=deployed_model_id,
        traffic_split=traffic_split,
    )
    print("Long running operation:", response.operation.name)
    undeploy_model_response = response.result(timeout=timeout)
    print("undeploy_model_response:", undeploy_model_response)


# [END aiplatform_v1beta1_undeploy_model_sample]
